...
import { ClipboardCheck, Check, X, Radio, MapPin } from "lucide-react";
import { toast } from "sonner";

const Approval = () => {
  const [pendingDevices, setPendingDevices] = useState<Device[]>(
    mockDevices.filter((d) => d.status === "pending")
  );
  ...
  const handleApprove = () => {
    ...
    const nextPort = 9000 + mockDevices.filter(d => d.scadaPort).length + 1;
    toast.success(`${formData.name} aprovado → Porta ${nextPort}`);
    ...
    <DashboardLayout>
      <div className="mb-6">
        ...
        <p className="text-xs text-muted-foreground">
          Dispositivos aguardando identificação e binding de porta SCADA
        </p>
      </div>
                  ...
                  >
                    ...
                    <div className="mt-2 grid grid-cols-2 gap-2 text-[11px]">
                      ...
                      <div>
                        ...
                        <span className="ml-1 font-mono tabular-nums text-foreground">{device.lastIp}</span>
                      </div>
                    </div>
                  </motion.button>
                ))}
            ...
            >
              <div className="mb-4 rounded-md bg-secondary p-3">
                <p className="text-[11px] text-muted-foreground">
                  ...
                  IP: <span className="font-mono tabular-nums text-foreground">{selectedDevice.lastIp}</span>
                  <span className="mx-2">|</span>
                  Sinal: <span className="tabular-nums text-foreground">{selectedDevice.signal} dBm</span>
                </p>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <select
                    value={formData.type}
                    onChange={(e) => setFormData({ ...formData, type: e.target.value as "RTU" | "CLP" | "Modem" })}
                ...
                <div className="flex gap-2 pt-2">
                  <button
                    ...
                    className="flex h-9 flex-1 items-center justify-center gap-2 rounded-md bg-primary text-xs font-medium text-primary-foreground transition-colors hover:bg-primary/90"
                  >
                  ...
                  <button
                    ...
                    className="flex h-9 items-center justify-center gap-2 rounded-md border border-border px-4 text-xs font-medium text-destructive transition-colors hover:bg-destructive/10"
                  >
